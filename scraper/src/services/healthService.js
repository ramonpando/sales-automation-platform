...
class HealthService {
  constructor() {
    ...
    // Dependencias que se cargarán de forma perezosa (lazy-loading)
    this.logger = null;
    this.database = null;
    this.redis = null;

    // =============================================
    // BINDING 'this' CONTEXT (SOLUCIÓN AL ERROR DE INICIALIZACIÓN)
    // =============================================
    // Esto asegura que 'this' siempre se refiera a la instancia de HealthService,
    // sin importar cómo se llamen los métodos desde fuera.
  ...
  async initialize() {
    ...
    try {
      ...
      this.registerDefaultHealthChecks(); // 'this' ahora es correcto gracias al .bind()
  ...
  async stop() {
    ...
    try {
      logger.info('Performing final health check before shutdown...');
      await this.performHealthCheck();
    } catch (error) {
